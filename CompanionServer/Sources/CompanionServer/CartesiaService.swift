...
/// Persistent WebSocket to Cartesia Sonic. Text chunks stream in per turn (`context_id`);
/// PCM audio chunks stream back incrementally.
actor CartesiaService: TTSStreamingService {
    ...
    private func connectIfNeeded() async throws {
        ...
        task.resume()
        socket = task
        logger.info("cartesia ws connected", metadata: ["model": .string(modelId)])
